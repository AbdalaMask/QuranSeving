...
namespace QuranLibrary
{
    public class QuranLibrary
    {
        ...
        public QuranCollection q { get; private set; }

        public QuranCollection TM { get; private set; } 
        ...
        public void LoadData()
        {
            ...
            using (StreamReader sr = new StreamReader(Path.Combine(appFolder, @"Data\ar.muyassar.xml")))
            {
                XmlSerializer sSerializer = new XmlSerializer(typeof(QuranCollection));
                TM = (QuranCollection)sSerializer.Deserialize(sr);
                //MessageBox.Show(q.surahs.Length.ToString());
